...
import { Download, RefreshCw, AlertTriangle, FileUp, Info, Search, Layers, Package, Image as ImageIcon, Music, Archive, X } from 'lucide-react';
...
export const RescueModal: React.FC<RescueModalProps> = ({ isOpen, onClose }) => {
    ...
    const downloadItem = async (item: BackupItem) => {
        try {
            ...
            if (data instanceof Blob) {
                ...
                saveAs(blob, `Rescue_${item.name.replace(/[^\w\s가-힣]/g, '_')}_${item.key.replace(/[^\w-]/g, '_')}.json`);
            }
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-5xl max-h-[90vh] bg-[#0f0f0f] border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col animate-in fade-in zoom-in duration-200">
                ...
                <div className="p-8 pb-4 flex items-start justify-between border-b border-white/5">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-[var(--color-primary)]/10 rounded-2xl flex items-center justify-center">
                            <AlertTriangle size={24} className="text-[var(--color-primary)]" />
                        </div>
                        <div>
                            <h1 className="text-2xl font-black italic tracking-tighter text-white uppercase">Rescue Center</h1>
                            {error ? (
                                <p className="text-xs text-red-500 font-bold flex items-center gap-1"><AlertTriangle size={12} /> {error}</p>
                            ) : (
                                <p className="text-xs text-gray-400 font-medium">
                                    브라우저 심부의 데이터를 <span className="text-[var(--color-primary)]">시각화</span>하여 긴급하게 추출하거나 직접 복구하세요.
                                </p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="px-4 py-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary)]/90 text-black text-xs font-bold rounded-xl transition-all flex items-center gap-2 shadow-lg shadow-[var(--color-primary)]/20"
                        >
                            <FileUp size={14} /> JSON 가져오기
                        </button>
                        <button onClick={onClose} className="p-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl transition-all">
                            <X size={20} />
                        </button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-hidden flex flex-col p-8 pt-6">
                    ...
                    <div className="flex flex-col md:flex-row gap-4 mb-6">
                        <div className="flex-1 relative">
                            ...
                            <input
                                ...
                                className="w-full bg-[#1a1a1a] border border-white/5 rounded-2xl py-3 pl-10 pr-4 focus:border-[var(--color-primary)]/50 focus:ring-1 focus:ring-[var(--color-primary)]/20 outline-none transition-all text-sm"
                                value={searchQuery}
                        ...
                        <div className="flex bg-[#1a1a1a] p-1 rounded-2xl border border-white/5 overflow-x-auto">
                            {[
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${activeCategory === cat.id ? 'bg-[var(--color-primary)] text-black shadow-lg shadow-[var(--color-primary)]/20' : 'text-gray-500 hover:text-gray-300'
                                        }`}
                                >
                                    {cat.icon} {cat.label}
                                </button>
                    ...
                    {/* List Box */}
                    <div className="flex-1 min-h-0 bg-[#161616] border border-white/5 rounded-3xl overflow-hidden shadow-inner flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-white/5">
                            {loading ? (
                                <div className="h-full flex flex-col items-center justify-center">
                                    <RefreshCw size={32} className="animate-spin mb-4 text-[var(--color-primary)]/50" />
                                    <p className="text-gray-500 font-bold animate-pulse text-sm">저장소 지도를 그리는 중...</p>
                                </div>
                            ) : filteredItems.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center opacity-30">
                                    <Layers size={48} className="mb-4" />
                                    <p className="text-sm font-bold">감지된 데이터가 없습니다.</p>
                                </div>
                            ) : (
                                filteredItems.map((item) => (
                                    <div key={`${item.source}-${item.key}`} className="p-4 px-8 hover:bg-white/[0.03] transition-colors flex items-center justify-between group">
                                        <div className="flex items-center gap-4 flex-1 min-w-0">
                                            <RenderThumbnail item={item} />
                                            <div className="min-w-0 pr-4">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className={`text-[9px] px-1.5 py-0.5 rounded font-black tracking-tighter ${item.category === 'projects' ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-800 text-gray-400'
                                                        }`}>
                                                ...
                                                <div className="flex items-center gap-3 text-[10px] font-mono text-gray-500">
                                                    ...
                                                    {item.owner && (
                                                        <span className="text-blue-400/80 font-bold bg-blue-500/5 px-1 rounded flex items-center gap-1">
                                                            <Package size={8} /> {item.owner.projectName}
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex items-center gap-6">
                                            <span className="text-[11px] font-mono font-bold text-gray-600 group-hover:text-gray-400 transition-colors">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 font-bold rounded-lg text-xs flex items-center gap-2 active:scale-95 transition-all text-gray-300"
                                                >
                ...
                {/* Footer */}
                <div className="p-8 pt-0 grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className="p-4 bg-white/[0.02] border border-white/5 rounded-2xl flex gap-3 items-start">
                        <Info className="text-blue-400 shrink-0" size={16} />
                        <div>
                            <h4 className="text-[10px] font-black text-gray-300 mb-1">복구 전문가 팁</h4>
                            <p className="text-[9px] text-gray-500 leading-normal">자산 수동 복구 시 파일명을 원래의 Key값으로 유지해야 프로젝트가 인식합니다.</p>
                        </div>
                    </div>
                    <div className="p-4 bg-white/[0.02] border border-white/5 rounded-2xl flex gap-3 items-start">
                        <ImageIcon className="text-[var(--color-primary)] shrink-0" size={16} />
                        <div>
                            <h4 className="text-[10px] font-black text-gray-300 mb-1">시각적 미리보기</h4>
                            <p className="text-[9px] text-gray-500 leading-normal">사진만 보고도 유실된 장면을 쉽게 찾을 수 있도록 썸네일을 제공합니다.</p>
                        </div>
                    </div>
                    <div className="p-4 bg-white/[0.02] border border-white/5 rounded-2xl flex gap-3 items-start">
                        <Archive className="text-purple-400 shrink-0" size={16} />
                        <div>
                            <h4 className="text-[10px] font-black text-gray-300 mb-1">프로젝트 패키지</h4>
                            <p className="text-[9px] text-gray-500 leading-normal">프로젝트 설정과 하위 이미지/음성 자산을 하나로 묶어 완벽하게 백업합니다.</p>
                        </div>
                    </div>
                </div>
