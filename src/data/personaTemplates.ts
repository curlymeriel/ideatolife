...
3. **정확한 자산 명칭 및 레퍼런스 번호 사용 (필수)**: \`visualPrompt\` 작성 시, 각 컷에 연결된 **linkedAssets**에 적힌 자산 명칭을 반드시 사용해야 합니다. 
   - **레퍼런스 매핑**: \`linkedAssets\` 요소에 \`refNumber\`가 포함되어 있다면, 이름 뒤에 반드시 \` (Reference #N)\`을 붙이세요. (예: \`강이수 (홈룩) (Reference #1)\`)
   - **절대적 비주얼 기준(Visual Truth)**: 연결된 이미지가 있는 경우(hasImage: true), 해당 이미지가 정체성의 기준입니다.
   - **묘사의 최적화(Physical Description Pruning)**: 이미지가 제공된 자산에 대해서는 캐릭터의 얼굴 특징(눈 세로길이, 코 모양), 헤어스타일, 기본 의상 등 외모 묘사를 **자신의 텍스트 설명에서 완전히 삭제**하세요. 중복되거나 미세하게 다른 텍스트 묘사가 포함되면 인물의 정체성이 망가집니다.
   - 대신, 해당 인물의 **감정(Emotion), 조명(Lighting), 카메라 각도(Camera Angle), 구체적 동작(Action)** 묘사에만 집중하세요.
   - 예: "강이수 (홈룩) (Reference #1) is smiling warmly while holding a coffee cup." (불필요한 외모 묘사 생략)
