...
export const Step2_Style: React.FC = () => {
    ...
    const handleSaveAsset = async (overrideData?: { description?: string, referenceImage?: string | null, draftImage?: string | null }) => {
        try {
            ...
            const { saveToIdb, generateAssetImageKey, resolveUrl } = await import('../utils/imageStorage');
            ...
            if (selectedAssetId === 'master_style') {
                if (setMasterStyle) {
                    // [FIX] idb:// URLs must be resolved to display immediately without refresh
                    const resolvedRef = finalRefUrl ? await resolveUrl(finalRefUrl) : null;
                ...
                // IMPORTANT: Sync local state to prevent UI reversion
                // [FIX] idb:// URLs must be resolved to display immediately without refresh
                const resolvedRef = finalRefUrl ? await resolveUrl(finalRefUrl) : null;
                const resolvedDraft = finalDraftUrl ? await resolveUrl(finalDraftUrl) : null;

                setDescription(currentDesc);
                setReferenceImage(resolvedRef);
                setDraftImage(resolvedDraft);
            }
            setIsEditing(false);
                        ...
                        <div className="flex items-center gap-3">
                            {!isEditing ? (
                                <button
                                    onClick={() => {
                                        setIsEditing(true);
                                        if (selectedAssetId !== 'master_style') setShowGenerationModal(true);
                                    }}
                                ...
                                >
                                    <Wand2 size={16} /> {selectedAssetId === 'master_style' ? 'Edit Definition' : 'Open Studio'}
                                </button>
                            ) : (
                                <>
                            ...
                            {selectedAssetId === 'master_style' ? (
                                <div className="flex gap-6 h-full min-h-0">
                                    <div className="flex-1 flex flex-col gap-4">
                                        <div className="glass-panel border border-[var(--color-border)] flex flex-col min-h-[300px]">
                                            <div className="flex items-center justify-between p-4 border-b border-[var(--color-border)]">
                                                <div className="flex items-center gap-2">
                                                    <Type size={18} />
                                                    <span className="text-sm font-bold uppercase">Master Style Description</span>
                                                </div>
                                                <button
                                                    onClick={handleMagicExpand}
                                                    disabled={isProcessing || !description}
                                                    className="px-3 py-1 bg-gradient-to-r from-[var(--color-primary)] to-[#FF9A5C] text-black text-xs font-bold hover:opacity-90 disabled:opacity-50 flex items-center gap-2 rounded-md transition-all shadow-sm"
                                                >
                                            ...
                                            <div className="p-4 flex-1">
                                                <textarea
                                                    className="w-full h-full bg-[var(--color-bg)] border border-[var(--color-border)] p-4 text-gray-300 focus:border-[var(--color-primary)] outline-none resize-none text-lg min-h-[200px]"
                                                    value={description}
                                                    onChange={(e) => setDescription(e.target.value)}
                                                    placeholder="전반적인 비주얼 스타일을 설명하세요. 이미지를 업로드하고 분석 기능을 사용할 수도 있습니다."
                                                />
                                            </div>
                                        </div>
                                        <div className="glass-panel p-4 border border-[var(--color-border)]">
                                            <label className="text-sm font-bold text-[var(--color-text-muted)] mb-2 block uppercase tracking-wider">Character Modifier (Optional)</label>
                                            <textarea
                                                value={characterModifier}
                                                onChange={(e) => setCharacterModifier(e.target.value)}
                                                className="w-full bg-[var(--color-bg)] border border-[var(--color-border)] p-3 text-gray-300 min-h-[80px] focus:border-[var(--color-primary)] outline-none resize-none"
                                                placeholder="캐릭터에게 공통적으로 적용될 스타일 수식어를 입력하세요."
                                            />
                                        </div>
                                        <div className="glass-panel p-4 border border-[var(--color-border)]">
                                            <label className="text-sm font-bold text-[var(--color-text-muted)] mb-2 block uppercase tracking-wider">Background Modifier (Optional)</label>
                                            <textarea
                                                value={backgroundModifier}
                                                onChange={(e) => setBackgroundModifier(e.target.value)}
                                                className="w-full bg-[var(--color-bg)] border border-[var(--color-border)] p-3 text-gray-300 min-h-[80px] focus:border-[var(--color-primary)] outline-none resize-none"
                                                placeholder="배경/장소에 공통적으로 적용될 스타일 수식어를 입력하세요."
                                            />
                                        </div>
                                    </div>
                                    <div className="w-1/3 glass-panel border border-[var(--color-border)] flex flex-col">
                                        <div className="flex items-center justify-between p-4 border-b border-[var(--color-border)]">
                                            <div className="flex items-center gap-2">
                                                <ImageIcon size={18} />
                                                <span className="text-sm font-bold uppercase">Reference Image</span>
                                            </div>
                                            {referenceImage && (
                                                <button onClick={() => { setReferenceImage(null); setDraftImage(null); }} className="text-[10px] text-red-400 hover:text-red-300 flex items-center gap-1">
                                                    <RotateCcw size={10} /> Clear
                                                </button>
                                            )}
                                        </div>
                                        <div className="flex-1 relative border-dashed border-[var(--color-border)] bg-[var(--color-bg)] group overflow-hidden">
                                            {referenceImage ? (
                                                <div className="h-full w-full relative">
                                                    <img src={referenceImage} className="w-full h-full object-contain opacity-70 group-hover:opacity-100 transition-opacity" />
                                                    <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 bg-black/40 gap-2 p-4 transition-opacity">
                                                        <button
                                                            onClick={handleExpandFromImage}
                                                            disabled={isProcessing}
                                                            className="w-full py-3 bg-white text-black text-sm font-bold hover:bg-gray-200 transition-colors flex items-center justify-center gap-2"
                                                        >
                                                            {isProcessing ? <Loader2 size={16} className="animate-spin" /> : <Wand2 size={16} />}
                                                            Expand Prompt from Image
                                                        </button>
                                                        <p className="text-[10px] text-white/70 text-center">이미지를 분석하여 전체 스타일 설명을 생성합니다.</p>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="h-full flex flex-col items-center justify-center text-[var(--color-text-muted)] p-6">
                                                    <Upload size={32} className="mb-3 opacity-50" />
                                                    <span className="text-xs text-center leading-relaxed">
                                                        AI 스타일 가이드 생성을 위한<br />
                                                        참조 이미지를 업로드하세요.
                                                    </span>
                                                </div>
                                            )}
                                            <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, 'reference')} className="absolute inset-0 opacity-0 cursor-pointer" />
                                        </div>
                                    </div>
                                </div>
                            ) : (
                    ...
                    config={{
                        ...
                        onSave: async (result: AssetGenerationResult) => {
                            await handleSaveAsset({
                                ...
                                referenceImage: referenceImage, // 기존 참조 이미지는 유지
                                draftImage: result.selectedDraft // 생성된 것은 확정 드래프트로 저장
                            });
