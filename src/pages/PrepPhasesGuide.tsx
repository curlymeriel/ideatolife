        ...
        <div className="min-h-screen bg-[#111111] text-white p-8">
            <div className="max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex flex-col gap-8 relative">
                    ...
                    <div className="hidden md:block absolute left-12 top-12 bottom-12 w-0.5 bg-gradient-to-b from-blue-500/20 via-purple-500/20 to-yellow-500/20 z-0" />

                    {phases.map((phase, index) => {
                        const [phaseNum, phaseName] = phase.title.split(': ');
                        return (
                            <div key={index}
                                className={`relative z-10 p-8 rounded-3xl border backdrop-blur-sm transition-all hover:translate-x-1 hover:shadow-2xl flex flex-col md:flex-row items-start gap-12 ${phase.color}`}
                            >
                                ...
                                <div className="flex flex-col gap-6 w-full md:w-[240px] flex-shrink-0">
                                    ...
                                    <div>
                                        <h2 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-1">{phaseNum}</h2>
                                        <h3 className="text-2xl font-bold leading-tight">{phaseName}</h3>
                                    </div>
                                    <button
                                        onClick={() => navigate(phase.nav)}
                                        className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl bg-[var(--color-primary)] text-black text-sm font-bold transition-all hover:brightness-110 group shadow-lg shadow-[var(--color-primary)]/10"
                                    >
                                        해당 단계로 이동
                                ...
                                <div className="flex-1 md:border-l border-white/10 md:pl-12 pt-1 mt-4 md:mt-0">
                                    <div className="mb-6">
                                        ...
                                        <ul className="flex flex-col gap-4">
                                            {phase.points.map((point, i) => (
                                                <li key={i} className="flex items-center gap-3 text-sm text-gray-300 group/item">
                                                    <div className="w-1.5 h-1.5 rounded-full bg-[var(--color-primary)] scale-75 group-hover/item:scale-100 transition-transform" />
                                                    {point}
                                                </li>
                                            ))}
                                        </ul>
                ...
                <div className="mt-20 py-16 text-center border-t border-white/5">
                    ...
                    <button
                        ...
                        className="px-16 py-5 bg-gradient-to-r from-[var(--color-primary)] to-orange-500 text-black font-extrabold rounded-2xl hover:opacity-90 transition-all shadow-2xl shadow-[var(--color-primary)]/30 text-xl"
                    >
                        Phase 1: 트렌드 분석 시작하기
                    </button>
                </div>
