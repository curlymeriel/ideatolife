...
${cuts.map((cut, i) => {
        ...
        return `# Cut ${i + 1}: ${duration}s (audio: ${hasAudio})
ffmpeg -loop 1 -t ${duration} -i ${num}_image.jpg ${cut.audioUrl ? `-i ${num}_audio.mp3 -c:a aac` : ''} -c:v libx264 -pix_fmt yuv420p -vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2" ${cut.audioUrl ? '-shortest' : ''} ${num}_segment.mp4`;
    }).join('\\n')}
